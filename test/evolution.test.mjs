...
import { allowsEvolutionTool, allowsMemoryDistillTool, EVOLUTION_SYSTEM, evolutionStatus, shouldAutoEvolve } from "../dist/agent/evolution.js";
...
test("self-evolution policy is evidence-gated and excludes autonomous code/prompt mutation", () => {
  ...
  assert.match(EVOLUTION_SYSTEM, /learning_capture/);
  assert.match(EVOLUTION_SYSTEM, /review candidate/);
...
test("self-evolution status explains mode, approval, and safety boundary", () => {
  ...
  assert.match(status, /requires confirmation/);
...
test("self-evolution runtime tools cannot mutate task state or browse the network", () => {
  assert.equal(allowsEvolutionTool("learning_capture", "off"), true);
  assert.equal(allowsEvolutionTool("memory_write", "off"), false);
  assert.equal(allowsMemoryDistillTool("memory_write"), true);
