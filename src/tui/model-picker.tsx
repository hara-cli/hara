...
export function movePicker(s: PickerState, key: "up" | "down" | "left" | "right", modelCount: number, style: ReasoningStyle): PickerState {
  if (key === "up" || key === "down") {
...
export function ModelPicker({
  ...
  useInput((_input, key) => {
    ...
    if (key.return) return onSelect(models[s.modelIdx] ?? current ?? "", s.effort);
    ...
    else if (key.leftArrow) setS((p) => movePicker(p, "left", models.length, style));
    else if (key.rightArrow) setS((p) => movePicker(p, "right", models.length, style));
  });
