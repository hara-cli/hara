...
| Remote mutation deduplication | Implemented after 0.166.1; approval and legacy-terminal coverage completed after 0.176.0 | `session.submit/send/steer/interrupt` and approval replies accept a client UUID, persist a prompt-free started receipt before model/tool execution, share concurrent work, save the first terminal result before ACK, replay across Serve restarts, and reject same-ID/different-payload reuse. Provider-owned Codex/Claude submit, steer, interrupt, approval replies, and legacy terminal input/key helpers use the same private ledger and publish committed/failed receipts. A crash-window or failed terminal save blocks later mutation until explicit authoritative recovery. Desktop retains only opaque retry IDs plus salted payload fingerprints and fences delayed steer/interrupt by turn ID. Controlled terminal streams additionally reject duplicate, conflicting, and out-of-order `inputSeq` values before the PTY write. |
...
The mobile bridge now persists bounded publication and command-receipt projections across restart, maps legacy
opaque Mobile command IDs to deterministic core UUIDs, and forwards approval decisions with their exact provider
session identity. Agent mailbox/follow-up delivery uses payload-bound command identity and turn fencing. Approval
replies and legacy terminal input/key helpers now cross the same write-ahead durable receipt boundary: exact retries
are no-ops, conflicting reuse fails closed, and an explicit denial can never become a remembered approval merely
because a stale client also sent `always=true`.
